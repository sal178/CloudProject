...
CREATE PROCEDURE UpdateMembers /* REUTURNS FLAG IF PROFILE ALREADY EXISTS ELSE CREATES USER PROFILE AND SETS FLAG */
@Username [varchar](30) ,
@FName [varchar](20) ,
@LName [varchar](20) ,
@UserPassword [varchar](30) ,
...
create procedure EnterRooms
   ...
   as 
   insert into rooms values(@room_no,@type,@price,@location,@TV,@fridge,@wifi,@availability);

   insert into rooms values(101,'Single',200,'Lahore','Yes','Yes','No','Yes');
   insert into rooms values(102,'Double',200,'Lahore','Yes','Yes','No','Yes');
   insert into rooms values(103,'Single',200,'Lahore','Yes','Yes','No','No');
   insert into rooms values(104,'Suite',250,'Karachi','Yes','Yes','No','Yes');
...
AS
Begin
	if(EXISTS(select * from rooms where rooms.Type_='Single' and rooms.Location='Lahore'))
	begin
	set @flag=1
